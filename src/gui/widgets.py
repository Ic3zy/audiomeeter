        ...
        if painter.begin(self):
            try:
                ...
                if add_page_height > 5:
                    ...
                    painter.drawText(
                        fixed_text_rect, Qt.AlignmentFlag.AlignCenter, fixed_text
                    )
...
class Circle_slider(QSlider):
    ...
    def __init__(
        self, min_val=-120, max_val=120, default_val=0, parent=None, in_circle_text=True
    ):
        super().__init__(parent)
...
class ToggleButton(QAbstractButton):
    ...
    @Property(QColor)
    def outlineOff(self):
        return self._outline_off

    @outlineOff.setter
    def outlineOff(self, c):
        self._outline_off = c
        self.update()

    @Property(QColor)
    def outlineOn(self):
        return self._outline_on

    @outlineOn.setter
    def outlineOn(self, c):
        self._outline_on = c
        self.update()

    @Property(QColor)
    def outlineHover(self):
        return self._outline_hover

    @outlineHover.setter
    def outlineHover(self, c):
        self._outline_hover = c
        self.update()
...
class Equalizer(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(
            QRect(-30, 0, self.width(), 10), Qt.AlignmentFlag.AlignCenter, "EQUALIZER"
        )
...
class Virtual_input(QWidget):
    def __init__(self, parent=None, slider_number=None):
        ...
        self.sliders_container = Slider_buttons_div(
            disable_led=True, slider_number=slider_number + 3
        )
...
class Hardware_panel(QWidget):
    def __init__(self):
        ...
        self.layout.addWidget(
            Cassette_player(), alignment=Qt.AlignmentFlag.AlignHCenter
        )
...
class AudioDeviceDialog(QDialog):
    def __init__(self, parent=None, is_outputs=True):
        ...
        self.setWindowTitle(
            "Select Device" if not self.is_outputs else "Select Input Device"
        )
        ...
        label_text = (
            "Select Output Device" if self.is_outputs else "Select Input Device"
        )
...
class Select_hardware_output_buttons(QWidget):
    def __init__(self, parent=None, slider_number=1, settings_button=False):
        ...
        self.text = f"A{slider_number}" if not settings_button else "⚙"
    ...
    def paintEvent(self, event):
        ...
        btn_rect = QRect(
            w_rect.left() + 1, w_rect.top() + 1, w_rect.width() - 2, w_rect.height() - 2
        )
...
class Select_h_i_container(QWidget):
    def __init__(self, parent=None):
        ...
        self.hardware_text.move(735, 30)

        self.layout.addWidget(
            self.h_o_button_container, alignment=Qt.AlignmentFlag.AlignTop
        )
...
class VirtualInputsContainer(QWidget):
    ...
    def paintEvent(self, event):
        ...
        aux_rect = QRect(
            w_rect.left() + half_width, title_rect.bottom() + 2, half_width, 14
        )
...
class MicsContainer(QWidget):
    ...
    def paintEvent(self, event):
        ...
        top_rect = QRect(
            w_rect.left() + padding_left, w_rect.top() + top_offset, content_width, 16
        )
        painter.drawText(
            top_rect,
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
        ...
        bottom_rect = QRect(
            w_rect.left() + padding_left, top_rect.bottom() + 2, content_width, 14
        )
        painter.drawText(
            bottom_rect,
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
